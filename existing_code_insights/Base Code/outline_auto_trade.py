...
def get_sentiment(i):
    ...
    for article in all_articles:
        ...
        if t_time(0, 0) <= published_time.time() <= t_time(13, 30):  # Filter articles based on time
...
read_my_list = pd.read_csv("#tickercsv") #####CSV OF STOCK TICKERS (OUTLINE ONLY)#####
stocks = read_my_list['Ticker'].tolist()

reduced_stocks = []


def stock_check(base=0, limit=0):
    for i in stocks:
    ...
    if reduced_stocks:
        with ThreadPoolExecutor(max_workers=len(reduced_stocks)) as executor:
            ...
            for i in reduced_stocks:
                ...
                executor.submit(auto_buy, i, shares_to_buy, 0, 0)
...
#####IMPORTANT##### LINES 151-152 - SCHEDULE BASED ON TIME ON LOCAL MACHINE