...
def news_score(ticker):
    ...
    for article in all_articles:
        ...
        if t_time(0, 0) <= published_time.time() <= t_time(13, 30):  #KEY# CCC
...
def backtest(base, limit, buying_power, added_limit, prevent_loss):
    read_my_list = pd.read_csv('#enter csv here') ###########CSV WITH STOCK TICKERS###########
    ...
    for ticker in reduced_stocks:
        ...
        try:
            ...
            timeset_fallback = settings.timestamp_fallback13
            bars_filter_fallback = bars_df[bars_df.index == timeset_fallback]['open']
            price = (bars_filter_fallback.iloc[0])
            print(f"${price} for share of {ticker} at {timeset_fallback}")
        ...
        try:
            ...
            start_set_fallback = settings.timestamp_fallback13
            bars_filter_fallback = bars_df[bars_df.index == start_set_fallback]['open']
            check_price = (bars_filter_fallback.iloc[0])
...
if __name__ == "__main__":
    profit = backtest(0, 0,  100000, 0, 0) #KEY# AAA
...
###IMPORTANT - BBB### settings.py VARIABLES: BASE_DATE & TIMESTAMP_13/20/fallback13
...
# timestamp_fallback13 - fallback buying time (ERRORS WILL OCCUR IF CHANGED)
