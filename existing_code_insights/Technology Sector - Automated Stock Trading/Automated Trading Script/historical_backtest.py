...
def fetch_historical_data(ticker, start_date, end_date, interval='2m'):
    ...
    """
    try:
        # Minimal buffer - only fetch the exact dates requested
    ...
    try:
        ...
        for timestamp, row in future_data.iterrows():
            ...
            if low <= stop_loss_price and high >= take_profit_price:
                ...
                # Ensure both times are in same timezone for accurate calculation
                import pytz
                et_tz = pytz.timezone('America/New_York')
                
                if hasattr(entry_time, 'tz') and entry_time.tz is not None:
                    entry_time_et = entry_time.astimezone(et_tz)
                else:
                ...
                holding_minutes = (timestamp_et - entry_time_et).total_seconds() / 60
                profit_loss = exit_price - entry_price
                # Percentage relative to the position capital
                shares = int(investment_per_stock / entry_price)
                position_capital = entry_price * shares if shares > 0 else investment_per_stock
                profit_loss_pct = ((profit_loss * shares) / position_capital * 100) if position_capital else 0.0
...
def simulate_trade_execution(entry_price, stop_loss_pct, take_profit_pct, price_data, entry_time, 
                           slippage=DEFAULT_SLIPPAGE, setup_delay_seconds=DEFAULT_SETUP_DELAY_SECONDS,
                           ticker=None, overnight_manager=None, investment_per_stock=1_000_000, backtest_end_date=None):
    """
    ...
    try:
        ...
        # Get price data after entry time, but constrain to backtest period
        future_data = price_data[price_data.index > entry_time]
        
        # If backtest_end_date is provided, filter out data beyond the backtest period
        if backtest_end_date is not None:
            backtest_end_dt = datetime.strptime(backtest_end_date, '%Y-%m-%d') + timedelta(days=1)
            # Convert to timezone-aware if needed
            import pytz
            if future_data.index.tz is not None:
                et_tz = pytz.timezone('America/New_York')
                backtest_end_dt = et_tz.localize(backtest_end_dt)
            future_data = future_data[future_data.index <= backtest_end_dt]
                    ...
                    if not should_hold:
                        ...
                        if not prev_day_data.empty:
                            ...
                            if hasattr(exit_time, 'tz') and exit_time.tz is not None:
                                ...
                                exit_time_et = et_tz.localize(exit_time)
                            
                            holding_minutes = (exit_time_et - entry_time_et).total_seconds() / 60
                            profit_loss = exit_price - realistic_entry_price
                            # Calculate P&L as percentage of total capital (14M for 14 stocks)
                            investment_per_stock = 1_000_000  # $1M per stock
                            shares = int(investment_per_stock / realistic_entry_price)
...
def run_historical_backtest_with_overnight(params):
    ...
    try:
        ...
        print(f"🔧 BUG FIX: Ensuring clean backtest start - no position carryover from before {params['start_date']}")
        ...
        # BUG FIX: Store backtest boundaries to prevent position carryover
        backtest_start_date = start_dt.date()
        backtest_end_date = end_dt.date()
        print(f"🔧 BUG FIX: Backtest boundaries set: {backtest_start_date} to {backtest_end_date}")
        ...
        while current_date <= end_dt:
            ...
            if current_date.weekday() < 5:
                    ...
                    if not should_hold:
                        ...
                        try:
                            ...
                            if price_data is not None and not price_data.empty:
                                ...
                                # Handle timezone differences - ensure both times are in same timezone
                                import pytz
                                et_tz = pytz.timezone('America/New_York')
                                
                                # Normalize both times to ET timezone
                                if hasattr(position['entry_time'], 'tz') and position['entry_time'].tz is not None:
                                    entry_time_et = position['entry_time'].astimezone(et_tz)
                                else:
                                    entry_time_et = et_tz.localize(position['entry_time'])
                                
                                if hasattr(morning_time, 'tz') and morning_time.tz is not None:
                                    morning_time_et = morning_time.astimezone(et_tz)
                                else:
                                    morning_time_et = et_tz.localize(morning_time)
                                
                                holding_minutes = (morning_time_et - entry_time_et).total_seconds() / 60
                ...
                if available_stocks:
                    # BUG FIX: Only allow new positions on or after backtest start date
                    current_date_only = current_date.date()
                    if current_date_only < backtest_start_date:
                        print(f"🔧 BUG FIX: Skipping new positions on {current_date_only} - before backtest start {backtest_start_date}")
                        qualified_stocks = {}
                    else:
                    ...
                    if not should_hold_eod:
                        ...
                        try:
                            ...
                            if price_data is not None and not price_data.empty:
                                ...
                                if hasattr(eod_time, 'tz') and eod_time.tz is not None:
                                    eod_time_et = eod_time.astimezone(et_tz)
                                else:
                                    eod_time_et = et_tz.localize(eod_time)
                                
                                holding_minutes = (eod_time_et - entry_time_et).total_seconds() / 60
        ...
        for ticker, position in active_positions.items():
            try:
                ...
                if price_data is not None and not price_data.empty:
                    ...
                    total_capital = 14_000_000  # 14M total capital for 14 stocks
                    profit_loss_pct = (profit_loss / total_capital) * 100
                    # Handle timezone differences for backtest end - ensure both times are in same timezone
                    import pytz
                    et_tz = pytz.timezone('America/New_York')
...
def run_historical_backtest(params):
    ...
    try:
        ...
        if overnight_manager.enabled:
            print(f"🌙 OVERNIGHT HOLDING ENABLED - Using full overnight backtest")
            return run_historical_backtest_with_overnight(params)
        else:
            print(f"☀️ INTRADAY ONLY - Using FIXED backtest (no overnight)")
            # Use FIXED version for intraday-only testing
            from run_historical_backtest_FIXED import run_historical_backtest_with_overnight_FIXED as _fixed
            fixed_params = {
                'start_date': params['start_date'],
                'end_date': params['end_date'],
                'sentiment_threshold': params.get('sentiment_threshold', 0.2),
                'sentiment_min': params.get('sentiment_min_override', params.get('sentiment_threshold', 0.2)),
                'sentiment_max': params.get('sentiment_max_override', 1.0),
                'stop_loss_pct': params['stop_loss_pct'],
                'take_profit_pct': params['take_profit_pct'],
                'investment_per_stock': params['investment_per_stock'],
            }
            return _fixed(fixed_params)
    except Exception as e:
