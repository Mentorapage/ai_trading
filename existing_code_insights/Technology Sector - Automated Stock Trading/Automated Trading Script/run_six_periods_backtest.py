#!/usr/bin/env python3
"""
Six-Period Backtest Runner
Run 20 strategies over six 2-week periods and generate required CSV reports.
"""

print("🚀 Starting Six-Period Backtest Runner...")
print("📦 Loading imports...")

import os
import sys
print("✅ Basic imports loaded")

import pandas as pd
print("✅ Pandas loaded")

import logging
from datetime import datetime, date
from pathlib import Path
from typing import Dict, List, Tuple
import json
import subprocess
print("✅ All standard imports loaded")

# Add current directory to path for imports
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
print("✅ Path configured")

print("📊 Loading trading_core...")
from trading_core import load_stock_universe
print("✅ trading_core loaded successfully")

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('logs/six_periods_backtest.log'),
        logging.StreamHandler()
    ]
)

# Define the six periods (14 calendar days each, starting from the 7th)
PERIODS = [
    ("2025-01-07", "2025-01-20", "2025-01"),
    ("2025-02-07", "2025-02-20", "2025-02"),
    ("2025-04-07", "2025-04-20", "2025-04"),
    ("2025-06-07", "2025-06-20", "2025-06"),
    ("2024-10-07", "2024-10-20", "2024-10"),
    ("2024-12-07", "2024-12-20", "2024-12"),
]

# Define the 20 strategies (S01-S20)
STRATEGIES = [f"S{i:02d}" for i in range(1, 21)]

def create_strategy_config(strategy_id: str) -> Dict:
    """Create strategy configuration based on strategy ID"""
    
    # Extract strategy number
    strategy_num = int(strategy_id[1:])
    
    # Base configuration
    base_config = {
        'id': strategy_id,
        'stop_loss_pct': 3.0,  # 3% stop loss
        'sentiment_min': 0.10,
        'sentiment_max': 0.60,
    }
    
    # Strategy-specific take profit levels
    if strategy_num <= 5:
        base_config['take_profit_pct'] = 5.0
    elif strategy_num <= 10:
        base_config['take_profit_pct'] = 8.0
    elif strategy_num <= 15:
        base_config['take_profit_pct'] = 12.0
    else:  # S16-S20
        base_config['take_profit_pct'] = 15.0
    
    # Adjust sentiment ranges for some strategies
    if strategy_num in [3, 8, 13, 18]:
        base_config['sentiment_min'] = 0.20
        base_config['sentiment_max'] = 0.70
    elif strategy_num in [4, 9, 14, 19]:
        base_config['sentiment_min'] = 0.15
        base_config['sentiment_max'] = 0.65
    elif strategy_num in [5, 10, 15, 20]:
        base_config['sentiment_min'] = 0.25
        base_config['sentiment_max'] = 0.75
    
    return base_config

def run_period_backtest(start_date: str, end_date: str, period_name: str) -> Dict:
    """Run backtest for a specific period using the existing multi-strategy script"""
    
    logging.info(f"🚀 Starting backtest for period {period_name}: {start_date} to {end_date}")
    
    # Define output files for this period
    trade_log_file = f"logs/multi_strategy_{start_date}_{end_date}.csv"
    strategy_summary_file = f"reports/multi_strategy_summary_{start_date}_{end_date}.csv"
    ticker_summary_file = f"reports/multi_strategy_per_ticker_{start_date}_{end_date}.csv"
    
    # Run the existing multi-strategy backtest script
    try:
        cmd = [
            "python3", "run_multi_strategy_backtest.py",
            "--start", start_date,
            "--end", end_date,
            "--strategies", "ALL",
            "--trade-log", trade_log_file,
            "--strategy-summary", strategy_summary_file,
            "--ticker-summary", ticker_summary_file
        ]
        
        logging.info(f"Running command: {' '.join(cmd)}")
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        
        logging.info(f"✅ Completed backtest for period {period_name}")
        
        # Read the strategy summary file to get results
        if Path(strategy_summary_file).exists():
            return pd.read_csv(strategy_summary_file)
        else:
            logging.warning(f"Strategy summary file not found: {strategy_summary_file}")
            return pd.DataFrame()
        
    except subprocess.CalledProcessError as e:
        logging.error(f"❌ Error running backtest for period {period_name}: {e}")
        logging.error(f"STDOUT: {e.stdout}")
        logging.error(f"STDERR: {e.stderr}")
        return pd.DataFrame()
    except Exception as e:
        logging.error(f"❌ Unexpected error for period {period_name}: {e}")
        return pd.DataFrame()

def process_results_to_csv_format(results_df: pd.DataFrame, period_name: str) -> pd.DataFrame:
    """Process backtest results into the required CSV format"""
    
    csv_data = []
    
    # If results_df is empty, create empty results for all strategies
    if results_df.empty:
        for strategy_id in STRATEGIES:
            csv_data.append({
                'strategy_name': strategy_id,
                'pnl_usd': 0.0,
                'stop_loss_count': 0,
                'take_profit_count': 0,
                'sentiment_exit_count': 0,
                'period_end_exit_count': 0
            })
    else:
        # Process actual results
        for strategy_id in STRATEGIES:
            # Find the strategy in results
            strategy_row = results_df[results_df['strategy_id'] == strategy_id]
            
            if not strategy_row.empty:
                row = strategy_row.iloc[0]
                csv_data.append({
                    'strategy_name': strategy_id,
                    'pnl_usd': row.get('total_pnl_$', 0.0),
                    'stop_loss_count': row.get('sl_count', 0),
                    'take_profit_count': row.get('tp_count', 0),
                    'sentiment_exit_count': row.get('eod_count', 0),  # Assuming EOD includes sentiment exits
                    'period_end_exit_count': row.get('eod_count', 0)
                })
            else:
                # Strategy had no results
                csv_data.append({
                    'strategy_name': strategy_id,
                    'pnl_usd': 0.0,
                    'stop_loss_count': 0,
                    'take_profit_count': 0,
                    'sentiment_exit_count': 0,
                    'period_end_exit_count': 0
                })
    
    return pd.DataFrame(csv_data)

def main():
    """Main execution function"""
    
    logging.info("🎯 Starting Six-Period Backtest Runner")
    
    # Ensure reports directory exists
    reports_dir = Path("reports")
    reports_dir.mkdir(exist_ok=True)
    
    # Store all period results for aggregation
    all_period_results = []
    
    # Run backtest for each period
    for start_date, end_date, period_name in PERIODS:
        try:
            # Run backtest for this period
            period_results = run_period_backtest(start_date, end_date, period_name)
            
            # Process results to CSV format
            period_df = process_results_to_csv_format(period_results, period_name)
            
            # Save period CSV
            period_csv_path = reports_dir / f"period_{period_name}.csv"
            period_df.to_csv(period_csv_path, index=False)
            logging.info(f"📊 Saved period CSV: {period_csv_path}")
            
            # Store for overall aggregation
            all_period_results.append(period_df)
            
        except Exception as e:
            logging.error(f"❌ Failed to process period {period_name}: {e}")
            # Create empty results for this period
            empty_df = pd.DataFrame([{
                'strategy_name': strategy_id,
                'pnl_usd': 0.0,
                'stop_loss_count': 0,
                'take_profit_count': 0,
                'sentiment_exit_count': 0,
                'period_end_exit_count': 0
            } for strategy_id in STRATEGIES])
            
            period_csv_path = reports_dir / f"period_{period_name}.csv"
            empty_df.to_csv(period_csv_path, index=False)
            all_period_results.append(empty_df)
    
    # Generate overall summary (aggregated across all periods)
    if all_period_results:
        overall_df = pd.concat(all_period_results, ignore_index=True)
        overall_summary = overall_df.groupby('strategy_name').agg({
            'pnl_usd': 'sum',
            'stop_loss_count': 'sum',
            'take_profit_count': 'sum',
            'sentiment_exit_count': 'sum',
            'period_end_exit_count': 'sum'
        }).reset_index()
        
        # Save overall summary CSV
        overall_csv_path = reports_dir / "overall_summary.csv"
        overall_summary.to_csv(overall_csv_path, index=False)
        logging.info(f"📊 Saved overall summary CSV: {overall_csv_path}")
        
        # Generate global performance CSV (single row with totals)
        global_performance = pd.DataFrame([{
            'total_pnl_usd': overall_summary['pnl_usd'].sum(),
            'total_stop_loss_count': overall_summary['stop_loss_count'].sum(),
            'total_take_profit_count': overall_summary['take_profit_count'].sum(),
            'total_sentiment_exit_count': overall_summary['sentiment_exit_count'].sum(),
            'total_period_end_exit_count': overall_summary['period_end_exit_count'].sum()
        }])
        
        # Save global performance CSV
        global_csv_path = reports_dir / "global_performance.csv"
        global_performance.to_csv(global_csv_path, index=False)
        logging.info(f"📊 Saved global performance CSV: {global_csv_path}")
    
    logging.info("🎉 Six-Period Backtest Runner completed successfully!")
    
    # Print summary
    print("\n" + "="*80)
    print("📊 SIX-PERIOD BACKTEST COMPLETED")
    print("="*80)
    print(f"Generated files in {reports_dir}:")
    for period_name in [p[2] for p in PERIODS]:
        print(f"  • period_{period_name}.csv")
    print(f"  • overall_summary.csv")
    print(f"  • global_performance.csv")
    print("="*80)

if __name__ == "__main__":
    main()
